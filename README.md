...
새 오인식은 매주 나옵니다. 그때마다 코드를 고쳐 배포할 수는 없으니, **서버에서 한 줄이면 끝나도록** 해 뒀습니다.

```bash
menu-bot correct add 누륭지 누룽지 --apply
```

`--apply`를 붙이면 규칙을 저장하는 것으로 그치지 않고 **이미 저장된 이번 주 식단까지 그 자리에서 고칩니다**(OCR을 다시 돌리지 않으므로 몇 초면 끝납니다). 붙이지 않으면 다음 수집부터 적용됩니다.

| 명령 | 하는 일 |
|---|---|
| `menu-bot correct add <잘못> <올바름> [--apply]` | 규칙 추가 |
| `menu-bot correct test <문구>` | 저장하지 않고 결과만 미리 보기 |
| `menu-bot correct list` | 지금 적용 중인 규칙 |
| `menu-bot correct apply` | 저장된 식단에 지금 규칙을 다시 입히기 |

규칙은 `data/menu_corrections.txt`(git 제외)에 `잘못 = 올바름` 한 줄씩 쌓입니다. 파일을 직접 열어 고쳐도 되고, `#`으로 주석을 달아도 됩니다. **한 줄이 잘못돼도 그 줄만 버리고 나머지는 그대로 동작합니다** — 파일 전체가 깨지는 JSON 대신 이 형식을 쓴 이유입니다.

주의할 것은 **얼마나 좁게 적느냐** 하나입니다. 규칙은 이름 일부만 있어도 적용되므로, 다른 메뉴에도 들어갈 수 있는 글자라면 앞뒤를 함께 적어 좁히세요.

| | 예 | 결과 |
|---|---|---|
| 안전 | `누륭지 = 누룽지` | `누륭`은 한국어에 없어 오작동할 데가 없음 |
| 위험 | `나초 = 나쵸` | `나초칩`처럼 원래 맞는 이름까지 바뀜 |
| 안전 | `돈육나초 = 돈육나쵸` | 그 메뉴에만 걸림 |

두 글자 이하 규칙을 넣으면 `correct add`가 경고를 띄웁니다. 넣은 뒤 `menu-bot correct apply` 출력에 엉뚱한 메뉴가 바뀌지 않았는지 확인하면 됩니다.

코드에 고정된 목록(`SUBSTITUTIONS`)과 이 파일이 겹치면 **파일이 이깁니다.** 현장에서 방금 본 것이 1년치 조사보다 최신이기 때문입니다.
...
- **금요일 18:00부터 매시 정각** `menu-bot check-next-week`가 게시글 제목만 가볍게 확인합니다(이미지 다운로드 없음). 금요일 18~23시 → 토요일 종일 → 일요일 22시까지 끊김 없이 이어지고, 일요일 23시부터는 시간별 재시도(`menu-bot ensure-menu`)가 같은 일을 **금요일 23시까지 매시** 이어받습니다. 식단표는 보통 금요일 오후에 올라오므로 대개 첫 몇 회 안에 확보되고, 그 뒤로는 그룹웨어에 접속하지 않습니다.
...
- 한 번 확인되면 상태 파일(`data/next_week_watch_state.json`)에 대상 주차가 기록되어, 남은 매시 실행은 그룹웨어에 다시 접속하지 않고 즉시 종료합니다(주차가 바뀌면 다음 주에 자동으로 다시 확인).
...
- 시도마다 결과(`not_posted` / `error` / `ingest_failed` / `not_servable` / `title_unreadable` / `confirmed`)와 사유를 상태 파일에 남깁니다. 마감 점검과 연속 실패 경보가 "무엇이 몇 번 실패했는지"를 그대로 알릴 수 있게 하기 위함입니다.
- **게시글이 아직 없는 것(`not_posted`)과 제목을 못 읽은 것(`title_unreadable`)을 구분합니다.** 제목 형식이 바뀌어 주차를 읽어 내지 못하면 예전에는 "아직 게시되지 않음"으로 조용히 넘어가 그 주를 통째로 놓칠 수 있었습니다. 지금은 실패로 세어 경보 대상에 넣습니다.
...
| 수집이 **3회 연속 실패**한 직후 | 그룹웨어 접속·파싱이 계속 막힘 | 🚨 연속 실패 횟수와 최근 실패 사유, 진단 방법. 마감을 기다리지 않고 그 자리에서 보냅니다 |
| 일요일 22:00 | **마지막 시도까지 해 본 뒤** 여전히 미확보 | 시도 요약(사유별 횟수), 최근 8회 시도와 오류 메시지, 다음 조치 안내 |

마감 점검은 상태 파일만 보고 판정하지 않고, 그 자리에서 수집을 한 번 더 시도한 다음 판정합니다. 직전 회차가 몇 분 뒤에 성공하는 경우 헛경보를 먼저 보내게 되기 때문입니다.

**마감 알림은 수집의 끝이 아닙니다.** 일요일 22시에 미확보 알림이 나가도 23시부터 매시 재시도가 계속되고, 식단표를 읽어 내는 순간 평소와 같은 수집 완료 메일이 갑니다. 미확보 알림과 연속 실패 경보는 각각 주차당 한 번만 보냅니다.

연속 실패 경보(`STUCK_ALERT_AFTER`, 기본 3회)는 마감까지 기다리다 사정을 너무 늦게 아는 일을 막습니다. 2026-08-29 사내 공지 팝업이 게시판 진입을 막았을 때, 첫 실패부터 마감 알림까지 44시간 동안 34번을 헛돌면서도 아무 알림이 없었습니다. 게시글이 아직 없는 것(`not_posted`)은 실패로 세지 않으므로, 금요일 저녁의 정상적인 "아직 안 올라옴"으로는 경보가 울리지 않습니다.

모든 알림 본문에 **발신 호스트**를 적습니다. 같은 Gmail 계정으로 서버와 데스크톱이 함께 알림을 보낼 수 있어, 메일만 보고 어디서 나온 것인지 가릴 수 있어야 하기 때문입니다.
...
- 금요일 18시부터 일요일 22시까지는 주말 폴링 타이머가 매시 확인을 맡고 있어, 이 타이머는 그 시간대를 비워 두고 중복으로 그룹웨어를 두드리지 않습니다. 두 타이머가 이어 붙어 금요일 18시부터 다음 금요일 23시까지 매시 확인이 끊기지 않습니다.
...
**식단표는 올라왔는데 그 날 칸만 비어 있는 경우는 다르게 답합니다.** 기다려도 메뉴가 생기지 않기 때문입니다. 비어 있는 이유(공휴일·회사 자체 휴무·식당 사정)는 식단표에 적혀 있지 않아 봇이 알 수 없으므로 단정하지 않습니다. 실제로 2026-09-23은 회사 자체 휴무일, 09-24~25는 추석 연휴였지만 식단표상으로는 셋 다 똑같은 빈 칸이었습니다.

```text
09월 23일(수)은 식단표에 메뉴가 없습니다.
...
| `menubot-nextweek-friday.timer` | 매주 금요일 18~23시 매시 KST | 다음 주 게시글 확인 |
| `menubot-nextweek-weekend.timer` | 토 종일·일 22시까지 매시 KST | 다음 주 게시글 반복 확인(확인되면 즉시 종료) |
| `menubot-nextweek-deadline.timer` | 매주 일요일 22:00 KST | 마지막 시도 후 미확보면 알림 메일 발송(이후에도 재시도는 계속) |
