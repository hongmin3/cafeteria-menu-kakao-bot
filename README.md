...
| 뷰웍스 게시글 없이 지점 게시글만 올라옴 | 지점 식단을 끼니 단위로 합쳐 답하고(운영 여부가 갈리면 운영하는 쪽 메뉴), 지점명은 노출하지 않으며 뷰웍스 게시글은 계속 기다림 |
...
### 뷰웍스 게시글 없이 지점 게시글만 올라온 주차

2026-08-24 주차에 실제로 있었던 일입니다. [화성]·[안양] 게시글만 올라와 113건이 저장됐는데, "수집 완료" 알림이 나가고 폴링이 멈췄지만 챗봇은 공통 식단을 고르지 못해 "확인할 수 없어요"로 답했습니다. **확보 판정을 응답 생성과 같은 기준으로 하지 않아 알림과 실제 답이 어긋난 것**입니다.

두 지점 식단을 셀 단위로 대조해 보니 42칸 중 31칸이 완전히 같았고, 진짜 차이는 "어느 요일 조식을 쉬는가"뿐이었습니다(안양은 화요일, 화성은 목요일 조식 미운영). 나머지 차이는 전부 OCR 오인식이었습니다. 그래서 이렇게 처리합니다.

- 끼니별로 **운영/미운영이 같으면** 하나로 합칩니다(코너별로 더 풍부하게 읽힌 쪽 채택).
- **운영 여부가 갈리는 끼니는 운영하는 쪽 메뉴**를 보여줍니다. 다른 지점이 쉰다는 사실은 뷰웍스 사람에게 아무 정보가 아니지만, 메뉴 자체는 거의 같기 때문입니다.
- **어느 지점에서 왔는지는 사용자에게 노출하지 않습니다.** 판단은 서버가 끝냅니다.
- 답은 나가지만 **확보로 처리하지 않습니다**. 뷰웍스 게시글이 올라오면 그것으로 바꾸고 다시 알립니다. 그동안은 제목만 가볍게 확인하고(이미지 재다운로드 없음, 실측 8초) 새 게시글이 있을 때만 수집합니다.
